...
true_sample_nd <- likelihood_normal(true_params_nd)
true_data_nd <- c(mean(true_sample_nd), sd(true_sample_nd))
simulated_data_nd <- matrix(true_data_nd, nrow = num_particles, ncol = 2, byrow = T)
...
test_that('Dimensions of pertubations are correct', {
  expect_equal(dim(generate_pertubations(num_particles, particles, likelihood_samples_nd)), c(num_particles, 2))
})

test_that('Dimensions of particles are correct after updating', {
  expect_equal(dim(update_particles(0.1, particles, simulated_data_nd, likelihood_samples_nd, covariances_nd, num_particles)), dim(particles))
})
...
test_that('Dimensions of covariance matrices are correct', {
  expect_equal(dim(cov(likelihood_samples_nd)), c(2, 2))
  expect_equal(dim(cov(particles)), c(2,2))
  expect_equal(dim(cov(particles, likelihood_samples_nd)), c(2, 2))
  expect_equal(dim(test_conditional_cov(particles, likelihood_samples_nd)), c(2, 2))
})
...
test_that('ESS is between 1 and N', {
  ...
  ll_densities = calculate_pseudo_densities(true_data_nd, num_particles, likelihood_samples_nd, diag(2))
