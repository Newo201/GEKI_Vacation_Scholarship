...
generate_results <- function(num_particles, true_parameters) {
  ...
  # Model 3
  eki_result <- eki_normal(num_particles, true_parameters, true_data, 
                           prior_params, adaptive = adaptive)
  plot_eki_normal(eki_result, true_parameters, prior_params)
  
  # MCMC Comparison for Model 3
  run.1 <- normal_mcmc(true_data, true_params, prior_params)
  chain.1 <- run.1$batch
  run.2 <- normal_mcmc(true_data, true_params, prior_params)
  chain.2 <- run.2$batch
  plot_mcmc_trace_plots(chain.1, chain.2, 500)
  plot_mcmc_histogram(chain.1, chain.2, true_params, 500)
