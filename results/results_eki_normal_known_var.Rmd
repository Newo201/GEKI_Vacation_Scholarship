...
plot_eki_normal_known_var <- function(eki_result, true_params, prior_params) {
  ...
  set.seed(2025)
  y <- likelihood_normal(true_params)
  ones_matrix <- x %*% t(x)
  
  post_mean = 25 * x %*% solve(25*ones_matrix + 4*diag(num_dimensions)) %*% t(y) 
  post_sd = sqrt(25 - 625 %*% t(x) %*% solve(25*ones_matrix + 4*diag(num_dimensions)) %*% x)
  
  alpha_post_density <- dnorm(alpha_sequence, mean = post_mean, sd = post_sd)
