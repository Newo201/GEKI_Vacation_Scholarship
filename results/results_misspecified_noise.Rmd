...
### Using Observations on Normal Scale

Previously we have used the model $log(y(t)) \sim N(log(W(t)-W(t-1)), \sigma^2)$, where our synthetic data was generated on the log scale. Instead we wish to look at $y(t) \sim logNormal(log(W(t)-W(t-1)), \sigma^2)$.
...
```{r}
eki_result <- eki_malaria(num_particles, true_data, true_unconstrained_params, prior_params, 
                          adaptive = F)
```
...
```{r}
plot(seq(0, 1, by = 0.1), dlogitnorm(seq(0, 1, by = 0.1)))
```

```{r}
particles <- initialise_malaria_particles(num_particles, prior_params)
hist(particles[, 3], freq = F)
hist(plogis(particles[, 3]), freq = F)
```

```{r}
eki_result <- eki_malaria_d_in_only(num_particles, true_data,
                                    true_unconstrained_params, general = T,
                                    prior_params, adaptive = adaptive)

```

```{r}
plot_eki_malaria_d_in_only(eki_result, true_params, prior_params)
```
