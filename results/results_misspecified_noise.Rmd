...
```{r}
# EKI
eki_result <- eki_malaria_known_var(num_particles, true_data, true_params, prior_params, general = F, adaptive = adaptive)
plot_eki_malaria(eki_result)

# GEKI
eki_result <- eki_malaria_known_var(num_particles, true_data, true_params, prior_params, adaptive = adaptive)
plot_eki_malaria(eki_result)
```
...
```{r}
sum(dnorm(c(1,2,3), mean = c(1,2,3), log = T))
```

```{r}
log(1/sqrt(2*pi))
dnorm(1, mean = 1, log = T)
```
