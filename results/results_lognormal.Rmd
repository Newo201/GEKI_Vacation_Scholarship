...
```{r}
source('C:/Users/owenj/OneDrive/Uni/Vacation Scholarship/GEKI_Vacation_Scholarship/src/models/eki_lognormal.R')
source('C:/Users/owenj/OneDrive/Uni/Vacation Scholarship/GEKI_Vacation_Scholarship/src/models/eki_lognormal_var_only.R')
```
...
generate_results <- function(num_particles, true_parameters) {
  ...
  print(c(mean(true_data), var(true_data)))
...
```{r, fig.dim = c(5, 2.5), fig.align = 'right'}
alpha_particles <- eki_result$particles[, 1]
sigma2_particles <- exp(eki_result$particles[, 2])
model <- lm(alpha_particles ~ sigma2_particles)
plot(alpha_particles, sigma2_particles, xlab = expression(alpha), 
     ylab = expression(sigma^2), main = 'Joint Posterior in Lognormal Model', 
...
```{r}
cor(alpha_particles, sigma2_particles)
```
