...
```{r}
plot_trace_plots <- function(chain.1, chain.2, burnin = 0) {
  
  par(mfrow = c(1,2))
  
  plot(chain.1[(burnin + 1):1000, 1], type = 'l', main = 'alpha')
  lines(chain.2[(burnin + 1):1000, 1], col = 'red')
  
  plot(exp(chain.1[(burnin + 1):1000, 2]), type = 'l', main = 'sigma2')
  lines(exp(chain.2[(burnin + 1):1000, 2]), col = 'red')
}
```

```{r}
plot_histogram <- function(chain.1, chain.2, true_params, burnin = 0) {
  chain.1.burnin <- chain.1[(burnin + 1): 1000, ]
  chain.2.burnin <- chain.2[(burnin + 1): 1000, ]
  ...
  hist(exp(combined_chains[, 2]), freq = F, main = 'sigma2')
  abline(v = true_params$sigma**2, col = 'red')
...
```{r}
prior_params = list(alpha.sd = 5, sigma2.sd = 2)
```
...
for (dimension in num_dimensions) {
  ...
  plot_histogram(chain.1, chain.2, 500)
}
```
