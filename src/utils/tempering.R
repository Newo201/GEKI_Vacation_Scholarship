...
get_weights <- function(next_temp, current_temp, simulated_data, likelihood_samples, num_particles) {
...
estimate_ess <- function(next_temp, current_temp, simulated_data, likelihood_samples, num_particles) {
  
  weights <- get_weights(next_temp, current_temp, simulated_data, likelihood_samples, num_particles)
  return(1/sum(weights**2))
}

get_ess_diff <- function(next_temp, current_temp, simulated_data, likelihood_samples, num_particles, target_ess) {
  
  ess <- estimate_ess(next_temp, current_temp, simulated_data, likelihood_samples, num_particles)
  return(ess - target_ess)
}

find_next_temp <- function(current_temp, simulated_data, likelihood_samples, num_particles, target_ess) {
  ...
  estimate_ess_partial <- partial(get_ess_diff, current_temp = current_temp,
                                  ...
                                  num_particles = num_particles, target_ess = target_ess)
