...
get_weights <- function(next_temp, current_temp, simulated_data, likelihood_samples) {
...
estimate_ess <- function(next_temp, current_temp, simulated_data, likelihood_samples) {
  
  weights <- get_weights(next_temp, current_temp, simulated_data, likelihood_samples)
  return(1/sum(weights**2))
}

get_ess_diff <- function(next_temp, current_temp, simulated_data, likelihood_samples, target_ess) {
  
  ess <- estimate_ess(next_temp, current_temp, simulated_data, likelihood_samples)
  return(ess - target_ess)
}

find_next_temp <- function(current_temp, simulated_data, likelihood_samples, target_ess) {
  
  # Fix the arguments of the estimate ess function
  estimate_ess_partial <- partial(get_ess_diff, current_temp = current_temp,
                                  simulated_data = simulated_data, likelihood_samples = likelihood_samples, target_ess = target_ess)
