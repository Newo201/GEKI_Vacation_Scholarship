...
densities_g_and_k_a_only <- function(true_data, num_particles, particles, parameters) {
  ...
  for (particle in 1:num_particles) {
    current_params = list(a = particles[particle, 1], b = parameters$b, 
...
synthetic_g_and_k_a_only <- function(num_particles, particles, parameters) {
...
synthetic_g_and_k_a_only_summary <- function(num_particles, particles, parameters) {
...
initialise_g_and_k_particles_a_only <- function(num_particles, parameters) {
...
eki_g_and_k_a_only <- function(num_particles, true_params, prior_params, adaptive = F) {
  ...
  if (adaptive) {
    return(eki_adaptive(num_particles, initial_particles, true_params, 
                        likelihood_g_and_k_summary, synthetic_g_and_k_a_only_summary, 
                        densities_g_and_k_a_only))
  }
  else {
