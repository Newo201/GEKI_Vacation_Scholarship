...
densities_g_and_k_g_only <- function(true_data, num_particles, particles, parameters) {
  ...
  for (particle in 1:num_particles) {
    current_params = list(a = parameters$a, b = parameters$b, 
                          g = particles[particle, 1], k = parameters$k)
...
synthetic_g_and_k_g_only <- function(num_particles, particles, parameters) {
...
synthetic_g_and_k_g_only_summary <- function(num_particles, particles, parameters) {
...
initialise_g_and_k_particles_g_only <- function(num_particles, parameters) {
  
  # Sample from the prior distribution
  prior_samples <- matrix(nrow = num_particles, ncol = 1)
  prior_samples[, 1] <- g_prior_sample(parameters, num_particles)
...
eki_g_and_k_g_only <- function(num_particles, true_params, prior_params, adaptive = F) {
  
  set.seed(2025)
  true_data <- likelihood_g_and_k_summary(true_params)
  set.seed(NULL)
  
  initial_particles <- initialise_g_and_k_particles_g_only(num_particles, prior_params)
  
  if (adaptive) {
    return(eki_adaptive(num_particles, initial_particles, true_data, synthetic_g_and_k_g_only_summary, 
                        densities_g_and_k_g_only))
  }
  else {
    return(eki(num_particles, initial_particles, true_data, synthetic_g_and_k_g_only_summary))
  }
