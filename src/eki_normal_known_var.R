...
parameters <- list(alpha = 2, sigma = 5, x = 1, alpha.sd = 2)
test <- initialise_particles(50, parameters)

# 
# num_observations <- 100
# simulated_data <- matrix(mean(likelihood_sample(parameters, num_observations)), nrow = 1, ncol = 1)
# # Sample from the prior distribution
# num_particles <- 1000
# x.true <- parameters$x
# sigma2.true <- parameters$sigma**2
# d_y = length(x.true)
# alpha.sd <- parameters$alpha.sd
# prior_samples <- matrix(nrow = num_particles, ncol = 1)
# prior_samples[, 1] <- alpha_prior_sample(alpha.sd, num_particles)
...
# particles <- prior_samples
# 
# likelihood_samples <- matrix(nrow = num_particles, ncol = dim(simulated_data)[1])
# 
# for (particle in 1:num_particles) {
#   
#   current_parameters <- list(alpha = particles[particle], x = x.true, sigma = sqrt(sigma2.true))
#   likelihood_samples[particle, ] <- mean(likelihood_sample(current_parameters, num_observations))
# }
#   
# C_yy <- cov(likelihood_samples)
# C_xx <- cov(particles)
...
# C_yx <- cov(likelihood_samples, particles)
# 
# # ToDo: fix the issue with this matrix being singular
# # H <- C_xy %*% solve(C_yy + 9*C_y_given_x)
# # H <- C_xy %*% solve(C_yy + sigma2.true*diag(num_observations))
# H <- C_xy %*% solve(C_yy + sigma2.true/num_observations*diag(1))
# 
# # pertubations <- rmvnorm(num_particles, mean = rep(0, num_observations), sigma = sigma2.true*diag(num_observations))
# pertubations <- rmvnorm(num_particles, mean = rep(0, 1), sigma = sigma2.true/num_observations*diag(1))
# 
# for (particle in 1:num_particles) {
#   
#   particles[particle, ] <- particles[particle, ] + H %*% (simulated_data - likelihood_samples[particle, ] - pertubations[particle, ])
# }
# 
# hist(particles, freq = F)
# particle_sequence <- seq(min(particles), max(particles), length.out = 20)
# 
# true_mean <- (alpha.sd^2)/(alpha.sd^2 + sigma2.true/num_observations)*simulated_data
# true_var <- alpha.sd^2 - alpha.sd^2*alpha.sd^2/(alpha.sd^2 + sigma2.true/num_observations)
# density <- dnorm(particle_sequence, mean = true_mean, sd = sqrt(true_var))
# density_prior <- dnorm(particle_sequence, mean = 0, sd = alpha.sd)
# lines(particle_sequence, density, col = 'red')
# lines(particle_sequence, density_prior, col = 'blue')
# 
# particles <- prior_samples
# 
# likelihood_samples <- matrix(nrow = num_particles, ncol = dim(simulated_data)[1])
...
# for (particle in 1:num_particles) {
#   
#   current_parameters <- list(alpha = particles[particle], x = x.true, sigma = sigma2.true)
#   likelihood_samples[particle, ] <- mean(likelihood_sample(current_parameters, num_observations))
# }
